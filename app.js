...
function __pushHistory(){
  try {
    ...
    __history.push(snap);
    if (__history.length > 80) __history.shift();
  } catch(e) { /* ignore */ }
}
function __undo(){
  if(__history.length === 0) return;
  const last = __history.pop();
  try { ctx.putImageData(last, 0, 0); } catch(e) {}
}
// --------------------------------
...
// === Mobile PNG Save: View-only (open image in new tab with <img>) ===
(function(){
  const btn = document.getElementById('savePngBtn');
  const canvas = document.getElementById('board');
  if(!btn || !canvas) return;
...
<title>PNG 미리보기</title>
<style>html,body{margin:0;padding:0;background:#111;height:100%;}img{display:block;max-width:100%;height:auto;margin:0 auto;}</style>
</head><body><img src="${dataURL}" alt="canvas preview"></body></html>`);
        w.document.close();
      } else {
