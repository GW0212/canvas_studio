...
(function(){
  try{
    if(!window.FIREBASE_CONFIG || !window.firebase) { console.warn('Realtime disabled: missing Firebase.'); return; }
    firebase.initializeApp(window.FIREBASE_CONFIG);
    const db = firebase.database();

    // Room handling via query ?room=1..10 (default 1)
    ...
    const canvas = document.getElementById('board');
    if(!canvas){ console.warn('No canvas#board found'); return; }
    const ctx = canvas.getContext('2d');

    // Utility (safe getters in case variables are local in original code)
    function getState(){
      // try known controls; if missing, use defaults
    ...
    // Wrapper helpers: hook existing functions if present
    const _startDraw = window.startDraw;
    const _moveDraw  = window.moveDraw;
    const _endDraw   = window.endDraw;

    let lastPt = null;
    let toolVal = (function(){
      const checked = document.querySelector('input[name="tool"]:checked');
      return checked ? checked.value : 'airbrush';
    })();
    document.querySelectorAll('input[name="tool"]').forEach(r=>{
      r.addEventListener('change', e=>{ toolVal = e.target.value; });
    });
    ...
    function applyOp(op){
      ...
      if(op.kind === 'path'){
        ...
        if(op.shape==='line'){
          ...
          // simple star
          const rOuter = Math.hypot(op.x2-op.x1, op.y2-op.y1);
          const rInner = rOuter*0.5;
          ctx.beginPath();
          ...
          for(let i=0;i<10;i++){
            const r = (i%2===0)?rOuter:rInner;
            const a = -Math.PI/2 + i*step;
            const x = op.x1 + r*Math.cos(a);
            const y = op.y1 + r*Math.sin(a);
            if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);
          }
          ctx.closePath();
          if(op.fill) ctx.fill(); else ctx.stroke();
          ...
          ctx.lineTo(op.x2 + Math.cos(a1)*headLen, op.y2 + Math.sin(a1)*headLen);
          ctx.lineTo(op.x2 + Math.cos(a2)*headLen, op.y2 + Math.sin(a2)*headLen);
          ctx.closePath();
          if(op.fill) ctx.fill(); else ctx.stroke();
        }
        ...
        ctx.globalCompositeOperation = 'source-over';
        ctx.clearRect(0,0,canvas.width,canvas.height);
        // try to repaint white bg if the app uses it
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(0,0,canvas.width,canvas.height);
      }
    ...
    // Wrap original handlers (if exist) to emit events **after** normal drawing
    if(typeof _moveDraw === 'function'){
      window.moveDraw = function(e){
        const prevLast = (window.lastX!==undefined && window.lastY!==undefined) ? {x: window.lastX, y: window.lastY} : null;
        _moveDraw.call(this, e);
        try{
          // derive current pointer from public getPos or compute manually
          if(prevLast && typeof getPos === 'function'){
            const pos = getPos(e);
            // Only emit when mouse is down; check isDrawing flag if present
            if(window.isDrawing || (e.buttons&1)){
              emitPath(prevLast, pos);
            }
          }
    ...
    if(typeof _endDraw === 'function'){
      window.endDraw = function(e){
        try{
          // on end, emit a shape if a preview mode was used
          const s = document.querySelector('input[name="tool"]:checked');
          const t = s ? s.value : toolVal;
          if(['line','rect','circle','star','arrow'].includes(t)){
            // use globally tracked startX/startY/lastX/lastY if they exist
            if(window.startX!==undefined && window.startY!==undefined && window.lastX!==undefined && window.lastY!==undefined){
              emitShape(t, window.startX, window.startY, window.lastX, window.lastY);
            }
          }
        }catch(err){ console.warn('emit shape fail', err); }
        _endDraw.call(this, e);
      }
    }

    // Admin reset hook: when resetBtn clicked in admin mode, also broadcast clear
    const resetBtn = document.getElementById('resetBtn');
    if(resetBtn){
    ...
    console.warn('Realtime wrapper error', e);
  }
})();
